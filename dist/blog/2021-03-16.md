# Nginx：各类配置
-----------------------
<br>

标签 ：`Nginx`   日期 ：`2021-03-16`
-----------------------
## 常规配置
```yaml
stream {
    include vhost/streambase.conf;
    include vhost/blackip.conf;
    include vhost/stream.conf;
}

http {

    include vhost/httpbase.conf;
    include vhost/blackip.conf;
    include vhost/http.conf;

    server{
        listen 9001;
        location /flv/ {
            proxy_pass  http://192.168.1.201:9001/flv/;
        }
        location / {
            return  403;
        }
    }
}
```

## tcp全局配置-streambase.conf
```yml
    map $time_iso8601 $logdate {
		'~^(?<ymd>\d{4}-\d{2}-\d{2})' $ymd;
		default    'date-not-found';
	}
    log_format main2 '$remote_addr [$time_local] '
                 '$protocol $status $bytes_sent $bytes_received '
                 '$session_time "$upstream_addr" '
                 '"$upstream_bytes_sent" "$upstream_bytes_received" "$upstream_connect_time"';
    access_log log/tcp-access-$logdate.log main2 ;
```

## http全局配置-httpbase.conf
```yml
server_tokens off;
client_max_body_size 10240m;

# daily log files
map $time_iso8601 $logdate {
    '~^(?<ymd>\d{4}-\d{2}-\d{2})' $ymd;
    default    'date-not-found';
}
access_log logs/access-$logdate.log;
```

## 黑名单限制-blackip.conf
```yml
    deny 59.202.18.199;
```

## 解除跨域
```yml
    server{
        listen 9001;
        location /flv/ {
            #cros
            if ($request_method = 'OPTIONS') {
                add_header Access-Control-Allow-Origin *;
                add_header Access-Control-Allow-Methods GET,POST,PUT,DELETE,PATCH,OPTIONS;
                add_header 'Access-Control-Allow-Headers' 'range,accessToken';
                return 200;
            }
            fastcgi_hide_header Access-Control-Allow-Origin;
            proxy_hide_header Access-Control-Allow-Origin;
            add_header 'Access-Control-Allow-Origin' "$http_origin";
            add_header 'Access-Control-Allow-Headers' 'range,accessToken';
            # no cache
            add_header 'Pragma' 'no-cache';
            add_header 'Cache-Control' 'must-revalidate';
            add_header 'Cache-Control' 'no-cache';
            add_header 'Cache-Control' 'no-store';
            expires 0;
            
            proxy_pass  http://192.168.1.201:9001/flv/;
        }
        location / {
            return  403;
        }
    }
```

## 错误跳转登陆页
```yml
    server{
        listen 9001;
        location /flv/ {
            proxy_pass  http://127.0.0.1:9001/flv/;
        }
        location / {
            rewrite ^/(.*)$ /404.html;
        }
        error_page 404 /404.html;
        location = /404.html {
            allow   all;
            rewrite ^/(.*)$ /;
        }
    }
```

## 自定义错误页面
```yml
    server{
        listen 9001;
        location /flv/ {
            proxy_pass  http://127.0.0.1:9001/flv/;
        }
        location / {
            rewrite ^/(.*)$ /404.html;
        }
        error_page 403 /403.html;
        location = /403.html {
            allow   all;
            root html/;
        }
    }
```



## Https配置
```yml
    # https
    server {
        listen 443 ssl;
        server_name  hsy.xxx.com;
        
        # zhengshu 1
        ssl_certificate   cert/2765227__cjbe88.com.pem;
        ssl_certificate_key  cert/2765227__cjbe88.com.key;
        ssl_session_timeout 5m;
        ssl_ciphers ECDHE-RSA-AES128-GCM-SHA256:ECDHE:ECDH:AES:HIGH:!NULL:!aNULL:!MD5:!ADH:!RC4;
        ssl_protocols TLSv1 TLSv1.1 TLSv1.2;
        ssl_prefer_server_ciphers on;
        # zhengshu 2
        ssl_certificate cert/gangyitong_zhengshu.crt;
        ssl_certificate_key cert/gangyitong_miyao.key;
        
        listen       19001;
        
        location /{
            root   D:\wwwroot\web;
            index  index.html index.htm;
        }       
    }
```

## 负载均衡
```yml
    # load balancing
    proxy_cache_path /nginx-1.20.2/cache-geoserver levels=1:2 keys_zone=geoserver_cache:5m max_size=1g inactive=30s use_temp_path=off;
    upstream geoserver_ship {
        server 172.16.30.103:8082 weight=1;
        server 172.16.30.104:8080 weight=1;
        server 172.16.30.101:8082 backup;
    }
    server {
        listen       8080;
        location / {
            proxy_cache geoserver_cache;
            proxy_cache_valid 200 304 12h;
            proxy_cache_valid any 15s;
            proxy_cache_key $host$uri$is_args$args;
            proxy_pass   http://geoserver_ship;
        }
    }
```
## http请求头比较
```yml
    server{
        listen 26223;
        location / {
            if ($http_Authorization != "Bearer sk-lm-AB3U8Jri:gtbT17CAvWuIbLeDbbc1") {
                return 403;
            }
            proxy_pass  http://127.0.0.1:16223/;
        }
    }
    
```

## 动态转发ip和端口
```yaml
location /hls/video/ {
    proxy_redirect  off;
    rewrite ^/hls/video/(.*)/po/(.*)/pa/(.*)$  /hls/video/$1/po/$2/pa/$3  break;
    proxy_pass http://$1:$2/hls/$3;
    add_header 'Access-Control-Allow-Origin' *;
    add_header 'Access-Control-Allow-Credentials' 'true';
    add_header 'Access-Control-Max-Age' 86400;
    add_header 'Access-Control-Allow-Headers' 'Authorization,Content-Type,Accept,Origin,User-Agent,DNT,Cache-Control,X-Mx-ReqToken,X-Requested-With';
    add_header 'Access-Control-Allow-Methods' 'GET,POST,OPTIONS';
    expires off;
    add_header Cache-Control "no-store, no-cache, must-revalidate, proxy-revalidate, max-age=0";
}
```

## Nginx转发TCP流，获取真实IP
```yaml
server{
  listen 4001; 
  proxy_pass 192.168.1.55:4001;
  proxy_protocol on;// 就是控制是否传送真实IP的
 }
```
> 客户端首次连接上来的时候，nginx会发送一个proxy 为前缀的包，里面包含原始IP和端口

> PROXY TCP4 192.168.1.2（真实ip） 192.168.2.3（多个转发ip） 1（真实端口） 2（多个转发端口)\r\n

> https://www.haproxy.org/download/1.8/doc/proxy-protocol.txt